 ...
 * Description: Handles doctor session management, dashboard metrics, 
 * appointment tracking, and medical record creation.
 */
...
// ==========================================
// 3. NAVIGATION & SESSION CONTROL
// ==========================================

/**
 * Validates doctor login session
 */
...
/**
 * Switches between main dashboard sections
 */
...
// ==========================================
// 4. PAGE RENDERING - DASHBOARD
// ==========================================

function renderDashboard() {
  if (!currentDoctorData) return;

  const appointments = DB.get("appointments").filter(a => a.doctor === currentDoctorData.name);
  const patients = DB.get("patients");
  const records = DB.get("records").filter(r => r.doctor === currentDoctorData.name);

  const today = getTodayDateString();
  const todayAppts = appointments.filter(a => a.date === today && !["Completed", "Cancelled", "No Show"].includes(a.status));
  const completedToday = appointments.filter(a => a.date === today && a.status === "Completed").length;

  const myPatientNames = new Set(appointments.map(a => a.patient).concat(records.map(r => r.patient)));
  const myPatientsCount = myPatientNames.size;

  // Render Stats Grid
  document.getElementById("stats-grid").innerHTML = `
    <div class="stat-card">
      <div class="stat-icon" style="background:#e3f4ff;font-size:22px"><span class="icon"><i class="fas fa-calendar-check"></i></span></div>
      <div class="stat-value">${todayAppts.length}</div>
      <div class="stat-label">Today's Appointments</div>
    </div>
    <div class="stat-card">
      <div class="stat-icon" style="background:#dcfce7;font-size:22px"><span class="icon"><i class="fas fa-check-circle"></i></span></div>
      <div class="stat-value">${completedToday}</div>
      <div class="stat-label">Completed Today</div>
    </div>
    <div class="stat-card">
      <div class="stat-icon" style="background:#fef9c3;font-size:22px"><span class="icon"><i class="fas fa-users"></i></span></div>
      <div class="stat-value">${myPatientsCount}</div>
      <div class="stat-label">Total Patients</div>
    </div>
    <div class="stat-card">
      <div class="stat-icon" style="background:#fee2e2;font-size:22px"><span class="icon"><i class="fas fa-file-medical"></i></span></div>
  ...
  if (todayAppts.length === 0) {
    todayContainer.innerHTML = `<div class="empty-appointments"><i class="fas fa-calendar-times"></i><p>No appointments scheduled for today</p></div>`;
  } else {
    todayContainer.innerHTML = todayAppts
      ...
      .map((appt) => {
        ...
        <div class="appointment-card">
          <div class="appointment-time"><div class="time-hour">${displayHour}:${minute}</div><div class="time-period">${period}</div></div>
          <div class="appointment-info"><div class="appointment-patient">${appt.patient}</div><div class="appointment-reason">${appt.reason}</div></div>
  ...
  const recentPatientNames = [...new Set([...appointments.slice(-5).map(a => a.patient), ...records.slice(-5).map(r => r.patient)])].slice(0, 5);
  const recentPatientsContainer = document.getElementById("recent-patients");
  const recentPatients = recentPatientNames.map(name => patients.find(p => p.name === name)).filter(Boolean);

  if (recentPatients.length === 0) {
    recentPatientsContainer.innerHTML = `<div class="empty-appointments"><i class="fas fa-user-injured"></i><p>No recent patients</p></div>`;
  } else {
    recentPatientsContainer.innerHTML = recentPatients.map((p) => {
...
function renderAppointments() {
  ...
  let appts = DB.get("appointments")
    ...
    .filter((a) => (!q || a.patient.toLowerCase().includes(q) || a.reason.toLowerCase().includes(q)) && (!f || a.status === f))
...
function renderPatients() {
  ...
  const myPatientNames = new Set([...appointments.map(a => a.patient), ...records.map(r => r.patient)]);
  ...
  tbody.innerHTML = myPatients.map((p) => `
    <tr>
      ...
      <td class="actions-cell"><button class="btn btn-ghost btn-sm" onclick="viewPatient(${p.id})">View</button></td>
    </tr>`).join("");
}

function viewPatient(id) {
  const p = DB.get("patients").find((x) => x.id === id);
  const records = DB.get("records").filter(r => r.patient === p.name && r.doctor === currentDoctorData.name);
...
function renderRecords() {
  ...
  let records = DB.get("records")
    ...
    .filter((r) => !q || r.patient.toLowerCase().includes(q) || r.diagnosis.toLowerCase().includes(q))
    .sort((a, b) => new Date(b.date) - new Date(a.date));
  ...
  tbody.innerHTML = records.map((r) => `
    <tr>
      ...
      <td class="actions-cell"><button class="btn btn-ghost btn-sm" onclick="viewRecord(${r.id})">View</button></td>
    </tr>`).join("");
}
...
function openRecordModal(id = null) {
  ...
  sel.innerHTML = '<option value="">Select Patient...</option>' + Array.from(myPatientNames).map(name => `<option>${name}</option>`).join("");

  document.getElementById("r-date").value = getTodayDateString();
  ["r-diagnosis", "r-prescription", "r-notes"].forEach(id => document.getElementById(id).value = "");
...
function saveRecord() {
  ...
  if (!patient || !diagnosis) return showErr("record-err", "Please fill in all required fields.");
...
function renderSchedule() {
  ...
  document.getElementById("current-schedule").textContent = currentDoctorData.schedule;
  document.getElementById("doctor-status").innerHTML = getStatusBadge(currentDoctorData.status);
  ...
  for (let i = 0; i < 7; i++) {
    ...
    const dayAppts = appointments.filter(a => a.date === dateStr && a.status !== "Cancelled").length;
...
function renderProfile() {
  ...
  const initials = currentDoctorData.name.split(" ").map(w => w[0]).join("").toUpperCase();
  ...
  document.getElementById("profile-phone").textContent = currentDoctorData.phone;
  document.getElementById("profile-email").textContent = currentDoctorData.email;
  document.getElementById("profile-license").textContent = currentDoctorData.license;
  document.getElementById("profile-schedule").textContent = currentDoctorData.schedule;
  document.getElementById("profile-status-badge").innerHTML = getStatusBadge(currentDoctorData.status);
}

// ==========================================
// 9. MODAL & OVERLAY CONTROLS
// ==========================================
